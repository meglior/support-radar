...
3.  **Web-UI:** Панель управления для инженеров с интеграцией Active Directory [7].

## 🛠 Технологический стек
*   **Backend:** Go (Core), PostgreSQL (Immutable Audit Log), Redis (Rate Limiting & Cache) [8].
*   **Agent:** Rust (Tokio, WinAPI), PowerShell (динамические модули) [1, 9].
*   **Безопасность:** mTLS 1.3, Ed25519 (Emergency Fallback), AES-256/RSA (шифрование логов) [10-12].

## 🚀 Ключевые возможности
*   **Hybrid Heartbeat:** Оптимизированный протокол обмена данными (~250–450 байт) для работы через медленные VPN-каналы [13].
*   **One-click Remediation:** Модуль автоматического исправления DNS, синхронизации времени и очистки дисков [14].
*   **Impersonation:** Безопасное выполнение пользовательских команд (Outlook, принтеры) через перехват токена активной сессии без использования SYSTEM-привилегий [6, 15].
*   **Anti-tampering:** Постоянный контроль целостности бинарного файла агента с мгновенным алертингом в SIEM [16, 17].

## 📁 Структура проекта (Go Server)
Согласно стандартам чистого кода, принятым в проекте [4, 18]:
...
## 🛡 Безопасность
Все коммуникации защищены двусторонним TLS. Клиентские сертификаты выпускаются автоматически через **AD Auto-Enrollment** [19, 20]. Для критических ситуаций предусмотрен **Emergency Fallback** сервер, работающий по модели статического манифеста, подписанного Root-ключом разработчика [12, 21, 22].
